...
If the --global (-g) flag is set, it applies configurations globally.`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			buildExtend(file.Command)

			buildCommand(file.Extend)
...
func psBlockStmtBuild(subCommand []psast.Stmt, ident string, cmd *cfg.Command) (bs []psast.Stmt) {
	...
	if len(cmd.Flags) > 0 {
		...
		for i, c := range match {
			...
			matchStmt.Cond = psast.Identifier(strings.Join(cases, " -and "))
			if len(c.body) > 0 {
...
func bashBlockStmtBuild(subCommand []bashast.Stmt, ident string, cmd *cfg.Command) (bs []bashast.Stmt) {
	...
	if len(cmd.Flags) > 0 {
		...
		for i, c := range match {
			...
			matchStmt.Cond = bashast.Identifier(strings.Join(cases, " && "))
			if len(c.body) > 0 {
				lines := strings.Split(c.body, "\n")
				for i, line := range lines {
					if i == len(lines)-1 && len(line) == 0 {
